  ...
  Sidebar = ({ activeTab, setActiveTab, mobileMenuOpen, user }) => {
    const menuItems = [
      ...
      },
      {
        ...
        active: activeTab === 'AnalyticsTraffic'
      }
    ];
              ...
              {menuItems.map(item => (
                ...
                >
                  <div className="flex items-center">
                    ...
                    <span className={`font-medium ${activeTab === item.id ? 'text-white' : 'text-gray-300 group-hover:text-white'
                      }`}>
                      {item.label}
                    </span>
                  </div>
                  <ChevronRight className={`w-4 h-4 transition-transform ${activeTab === item.id
